...
const signUpUser = async (req, res) => {
  ...
  try {
    ...
    const token = createToken(user._id);
    res.status(200).json({ email, token, pseudo, userId: user._id });
  } catch (error) {
    res.status(400).json({ error: error.message });
  }
};
...
const removeFollowing = async (req, res) => {
  ...
  try {
    console.log("first1");
    console.log(user1Id);
    const user1 = await User.removeFollowing(user1Id, user2Id);
    const user2 = await User.removeFollower(user1Id, user2Id);
    res.status(200).json({ user1, user2 });
  } catch (error) {
    res.status(400).json({ error: error.message });
  }
...
const getUserById = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Une erreur est survenue dans le getuserbyid." });
  }
};
...
const getIdByEmail = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: " Une erreur est survenue dans le getIdByEmail. " });
  }
};
...
const getPseudoById = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Une erreur est survenue dans le getPseudoById." });
  }
};
