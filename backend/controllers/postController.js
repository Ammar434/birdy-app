...
const listPost = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Une erreur est survenue dans la liste post." });
...
const newPost = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Une erreur est survenue dans le newPost." });
  }
};
...
const deletePost = async (req, res) => {
  ...
  try {
    ...
    console.error(error);
    res.status(500).json({
      message: "Une erreur est survenue dans la suppression du post.",
    });
  }
};
...
const likePost = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Une erreur est survenue dans le like Post." });
  }
};
...
const searchPostsandFriends = async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({
      message: "Une erreur est survenue dans la recherche Post and Friends.",
    });
  }
};
